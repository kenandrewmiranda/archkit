...
Harder / constrained: the chimera stands. `archkit doctor` run in a worktree with `ARCHKIT_ARCH_DIR` set can report goals from one project and hook wiring from another, and nothing in the human-readable `D-HOOKS` detail line currently names the `settings.json` it read — the path is in the JSON payload but not in the terminal output. Surfacing it there is a follow-up on `src/commands/doctor.mjs`, which this decision does not own and deliberately did not touch. Until then, the mitigation is this ADR plus the tests: `tests/archdir-resolution/run.mjs` §9 pins the divergence as intended, so a future reader who assumes it is a bug finds a red test naming the reason instead of a silent behavior change.
