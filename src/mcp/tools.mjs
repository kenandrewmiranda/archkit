...
export const tools = {
  ...
  archkit_finalize_config: {
    description: "Read or set the CGR finalization config (.arch/config.json → cgr.finalize) — the per-project policy for the wrap-up goal archkit_goal_intake auto-appends to every batch (changelog, docs, finalized commits, and the opt-in push / release / deploy-to-dev). This is the persistence side of the ONE-TIME setup: when intake returns `finalize.setup` (a project's first intake), present the steps to the user with AskUserQuestion (multi-select which to enable; ask if their CI/CD is github-actions or custom + a deploy command), then call this tool with their choices — it stamps `configured:true` so intake stops nudging. Call with no fields (or show:true) to read the current config. Defaults: changelog/docs/commit ON, push/release/deployDev OFF (outward-facing steps are a deliberate opt-in). To turn the whole feature off, call with enabled:false. archkit never runs git/deploy itself — the finalize goal carries the steps as exit-criteria and the agent executes the local ones + instructs the user for the rest. Returns { config, configured, nextStep } (or { saved, config, nextStep } on a write).",
    inputSchema: z.object({
  ...
  archkit_board_conflict: {
    description: "CGR 2.0 — RECORD a genuine cross-lane conflict and ESCALATE it to a merge-reconcile CGR (tier 3 of ADR 0013's hybrid conflict strategy). The strategy is three tiers in order: (1) pre-partition by ownership at intake, (2) worktree-isolate each lane, (3) ESCALATE what still collides. This is tier 3 — when two CGRs genuinely collide on file CONTENT during a convergence drain, call this instead of resolving it inline in the conductor's context. It appends a `conflict` board event AND mints a new CGR slugged `merge-reconcile-<slugA>-<slugB>` that depends_on every conflicting slug and is `exclusive`, so the lane partition schedules it as a SOLO BARRIER and the frontier withholds it until the colliding CGRs have completed. The minted CGR's body carries the conflicting slugs, the conflicting files, and the conflict provenance verbatim, so a fresh worker resolves it without re-deriving the collision. IDEMPOTENT: the reconcile slug is derived deterministically from the sorted conflicting slugs, so recording the same conflict twice appends a second event (the log is append-only by contract) but mints only ONE reconcile CGR (minted:false, reason already-queued). TERMINOLOGY — this is the MERGE sense of reconcile: conflicting file CONTENT. It is NOT archkit_goal_reconcile, which is the PLACEMENT sense (moving goal FILES into the folder their status dictates, ADR 0020/0021) and never touches file content or git; and it is NOT lane convergence (rebase-onto-tip before a lane lands, ADR 0023). archkit runs no git here — it records the collision and queues the work. Returns { slugs, files, event, reconcile:{ slug, minted, reason, dependsOn, exclusive, path }, pendingEscalations, nextStep }.",
    inputSchema: z.object({
  ...
  archkit_goal_start: {
    description: "CGR relay — mark a SPECIFIC goal in-progress by slug and return its payload. This is the actionable companion to the /mcp__archkit__conductor routing CHOICE: the conductor relay normally auto-picks the next goal, but when BOTH the ungrouped queue AND one or more project feature-sets (feat/<project>) have ready work, it surfaces a choice instead of guessing (cgr-relay-queue-vs-project-routing). After you present that choice to the user (AskUserQuestion) and they pick a track, call this with that track's recommended 'next' slug to begin it. Starting an UNGROUPED queue goal records the batch's shared cgr-queue-<date> branch (every plain queue goal reuses it); a project goal branches on feat/<project>. archkit only emits branch guidance in the payload — it never runs git. Use ONLY to resolve a routing choice or to deliberately start an out-of-order goal; for the normal one-keystroke advance the user runs /mcp__archkit__conductor. Returns { slug, status:'in-progress', payload, nextStep }.",
    inputSchema: z.object({
