...
export function genSystemMd(cfg) {
  ...
  o += `  - Write or update the corresponding test (unit for service, integration for controller)\n`;
  o += `  - Include seed data for any new DB tables (migrations/seed.ts)\n`;
  o += `  - Verify the feature works end-to-end: seed → API call → correct response\n`;
  o += `\`\`\`\n`;
  o += `\n## Definition of Done\n`;
  o += `A feature is NOT complete until:\n`;
  o += `- [ ] Unit tests pass for service layer logic\n`;
  o += `- [ ] Integration test hits the real API endpoint (not mocks) and verifies response\n`;
  o += `- [ ] Seed data exists for any new tables so the feature is testable immediately\n`;
  o += `- [ ] Error paths are tested (invalid input, not found, unauthorized)\n`;
  o += `- [ ] \`archkit review --staged\` passes with zero errors\n`;
  o += `- [ ] Health check endpoint reflects the new dependency (if any)\n`;
  o += `\n## Session Management\n`;
