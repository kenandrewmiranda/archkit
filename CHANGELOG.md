...
## Unreleased — API-doc hard gate + goals-layout reconcile + lane integration

Three independent, additive bodies of work. An **API-doc hard gate** stops any edit against an undocumented external API before it lands; a **goals-layout reconcile** treats the goals folder as a derived cache of each goal's `status` frontmatter and keeps the two in sync automatically; and **lane integration** closes CGR 2.0's open loop end to end — a dispatched lane is now claimed, tracked, converged onto the branch tip, verified with a concrete command, and landed through CI. All three are safe: the gate is a no-op when disabled and fails open, placement fixes are always reported, and every integration step archkit adds is *emitted guidance* — it still runs no git and no tests itself.
