...
## Unreleased — concurrency-safe shared state + API-doc hard gate + goals-layout reconcile + lane integration

Four independent, additive bodies of work. **Shared-state safety** closes every lost-update race in `.arch/` — CGR 2.0 spawns parallel workers and the Stop hook runs in every session at every turn-end, but until now nothing in archkit locked anything; **an API-doc hard gate** stops any edit against an undocumented external API before it lands; **a goals-layout reconcile** treats the goals folder as a derived cache of each goal's `status` frontmatter and keeps the two in sync automatically; and **lane integration** closes CGR 2.0's open loop end to end — a dispatched lane is now claimed, tracked, converged onto the branch tip, verified with a concrete command, and landed through CI. All four are safe: the write contract fails *open* so a stuck lockfile can never hang a turn-end, the gate is a no-op when disabled and fails open, placement fixes are always reported, and every integration step archkit adds is *emitted guidance* — it still runs no git and no tests itself.

### Added — a shared-state write contract (ADR 0030)

CGR state lives in two stores with opposite concurrency models, and only one was safe. `.arch/board/events.ndjson` is an append-only log with a pure fold — correct under any number of concurrent appenders. Goal frontmatter under `.arch/goals/**` is ADR 0003's declared source of truth, and **every** mutation of it was a lock-free read-modify-write. There was no locking anywhere in the codebase; the only atomic write was one hand-rolled tmp+rename used in exactly one place. Four concrete defects followed, all now closed.

- **`src/lib/fslock.mjs` — the primitive, with no new dependency.** `atomicWriteFileSync` / `atomicWriteJsonSync` write to a pid-tagged sibling temp and `rename` into place, so the temp and the target share a directory and the rename stays intra-device; a reader sees the old file or the new one, never a truncated one. The advisory lock is a lockfile created with `fs.openSync(…, 'wx')` — an atomic create-if-absent on POSIX and Windows alike — carrying the holder's pid, host, token and acquisition time, exposed as `withLock` / `withArchLock`. It is **reentrant within a process** (depth-counted, release-order-independent) and never across processes; it releases in a `finally`, so a throwing mutation cannot strand it.
- **Stale locks break on a TTL, and every break is reported.** A lockfile older than 30 s is removed and re-acquired, and the breaker returns `brokeStale` plus an `onEvent` record — a silent break is a bug. An unparseable lockfile is dated by its mtime, so corruption degrades to a break rather than a deadlock. TTL is the staleness signal, not pid liveness: pids are reused, and a shared tree can be reached by processes that cannot see each other's process table.
- **Fail OPEN on contention — but never on the write.** These mutators sit on the Stop hook and the MCP request path, so failing closed would convert a stuck lockfile into a hung turn-end in *every* session. Past the wait budget the mutation proceeds unlocked, flagged `failedOpen` with the blocking holder named and a `process.emitWarning` on stderr. Fail-open's worst case is exactly the previous behavior, and goal frontmatter is git-tracked so a lost update is recoverable. This applies to lock **acquisition** only — if the atomic write itself fails, the mutation fails loudly and never falls back to an in-place write.
- **`appendEvent` is deliberately NOT wrapped.** The board writes one small `O_APPEND` line per event, `readEvents` discards torn lines, and the fold is pure — it cannot lose an event to a concurrent appender. Wrapping it would serialize the system's highest-frequency write behind its slowest and give the one store that currently cannot lose data a new failure mode. The board stays lock-free and append-only.

### Fixed — every lost-update race in `.arch/` (ADR 0030)

- **`stampGoalFields` no longer loses a concurrent field write.** The funnel every lifecycle transition passes through was `loadGoal` → mutate → whole-file write, so a lease renewal and a status transition landing together were last-writer-wins over the whole file and one field vanished. It is now lock-**then**-load with an atomic write — the delta is applied to what is found inside the lock, never to a snapshot taken before it. Twelve mutation sites converted in total, including `runGoalComplete`'s archive and `reconcileGoalsLayout`'s apply pass.
- **Reclaim can no longer re-dispatch a live lane.** `reclaimExpiredLeases` folded the board to compute expiry, then stamped `lease: null` — a worker that renewed inside that window lost its lease anyway and its lane went out to a second worker. Expiry is now re-checked **inside** the lock, gating the `lease-expired` event as well as the stamp.
- **Concurrent Stop hooks can no longer clobber the digest.** Consolidation read the day's digest, deleted the source goal files, then wrote the whole digest back — two overlapping hooks both deleted, and the second write erased the first's entries unrecoverably. It is now structurally **append-only**: archival and claim are one atomic **claim-by-rename** (losers get `ENOENT` and emit nothing), and each entry is a single `O_APPEND` write. This path deliberately does not lean on the lock, precisely because the lock fails open. The on-disk digest format is byte-identical and still read by `listGoalDigests` and `archkit_goal_list`.
- **Loop state, queue branches and the chat board.** `writeLoopState`, `bumpLoopBlock`, `ensureQueueBranch`, `clearQueueBranchIfDrained` and `resetLoopState` now read inside the lock and replace atomically, so a concurrent turn-end cannot drop a turn-cap increment or a queue-branch record. `appendChatEntry` was converted to a genuinely append-only write (`O_CREAT|O_EXCL` to create, a single `appendFileSync` thereafter, no read on the write path), so two agents announcing at once can no longer lose an entry.
- **Goal proposals, graph proposals and `.arch/config.json`.** `writeGoalProposal` (its check-then-act dedup and its hand-rolled tmp/rename both retired), `writeGraphProposal`, `acceptGraphProposal` — including `appendValidatedNodeLine` writing the cluster `.graph` atomically — and `writeFinalizeConfig`'s merge-write all read inside the lock and replace atomically; `promoteGoalProposal` is locked as the two-file transaction it always was. The Stop hook now takes **one** acquisition for its whole deferred-goal detection batch (the lock is reentrant), instead of re-paying the fail-open wait per detection.
- **Proved with real processes, not mocks.** Seven new suites — `tests/fslock`, `tests/cgr-concurrency`, `tests/cgr-consolidate`, `tests/board-json-lock`, `tests/proposal-json-lock`, `tests/archdir-resolution`, `tests/doctor-hooks-path` — drive genuinely concurrent child processes released from a barrier against temp fixtures. Every race carries a **pre-fix negative control** that fails loudly if the workload is too weak to lose an update (the controls lose 15–25 graph node lines, erase the last recorded gap, revert 2–5 of 7 config knobs, and sometimes leave the file unparseable), plus source audits that go red if a future edit quietly drops a lock or a `writeFileSync` reappears against `.arch/`. Suite count: 77 → **84**.

### Added — explicit archDir resolution: `ARCHKIT_ARCH_DIR` (ADRs 0031, 0032)

Nothing resolved archDir once. Twenty private `findArchDir` copies were spread across `src/` and `bin/` with three signatures and two different existence checks — `shared.mjs` accepted a bare `.arch/` and capped the walk at 10 parents while the MCP and hook copies required `SYSTEM.md` and walked to the root, and `shared.mjs` silently *ignored* the argument a caller passed. So which `.arch/` a call touched was a function of where the process happened to start, and a caller had no way to say. This matters most for the conductor's worktree workers: `.arch/board/` is gitignored, so a worktree contains no board at all and an archkit run inside one answers against a fresh empty one. It also **gates ADR 0030** — two processes mutually exclude only if they resolve the same archDir, so a lock taken against a worktree's own `.arch/` excludes nothing.

- **One resolver, `src/lib/archdir.mjs`.** `resolveArchDir` / `requireArchDir` / `resolveArchDirForHook`, with precedence **explicit argument → `ARCHKIT_ARCH_DIR` → `process.cwd()` walked up to the nearest `.arch/`**. The MCP server, the CLI and all six hook bins call it; `tools.mjs` went from 49 private-walker calls to 49 shared-resolver calls with **zero** handlers re-deriving archDir. Ten duplicate walkers were deleted.
- **`ARCHKIT_ARCH_DIR` is now part of archkit's public interface.** Set, it wins outright and **no walk happens**. It names the `.arch` directory itself; a relative value resolves against cwd; a whitespace-only value reads as unset; and a value pointing at something that does not exist is an **error** (`invalid_arch_dir`), not a fallback — a caller that set the variable meant it, and degrading silently to cwd would recreate the accidental resolution this removes.
- **Worktree sharing is now deliberate and visible at the spawn.** The rendered dispatch step carries a `SPAWN-env` line with the conductor's real `.arch` path, and `archkit_conductor` returns `archDir` and `dispatch.env` structurally. A worker spawned with the variable reads and writes the conductor's board, goal tree and locks regardless of its own cwd; a worker spawned without it gets its own tree — still a legal mode, but a chosen one.
- **Back-compat asserted, not assumed.** With the variable unset, resolution is diffed against verbatim reproductions of *both* pre-ADR walkers across a layout matrix with positive controls, and `archkit decisions list --json` / `archkit prd check --json` are proved unchanged by spawning the real CLI. **One intended divergence:** the old 10-parent walk bound is gone — resolution now walks to the filesystem root.
- **A repo-wide guard blocks a third regression.** A shape-based audit fails the suite if any shipped file outside `src/lib/archdir.mjs` joins `".arch"` onto a directory and steps to its parent; rename, loop-form and recursion evasions are each asserted, with negative fixtures and a self-validating exception list that cannot be padded in advance.
- **`hooks-status` stays cwd-scoped — by decision (ADR 0032).** `projectClaudeDir` resolves a `.claude/` belonging to a *checkout*, not an `.arch/`, and `ARCHKIT_ARCH_DIR` makes no promise about its parent directory: `ARCHKIT_ARCH_DIR=/shared/specs/.arch` is legal, and following it would point `archkit_install_hooks --apply` at `/shared/specs/.claude` — a path no Claude Code session will ever load. Hooks are configuration of the checkout you are standing in, so that is what `D-HOOKS` answers about. The change was **comment-only** (proved mechanically) and the divergence is pinned by nine tests that go red if anyone routes it through the resolver.

### Changed — `archkit doctor` names the `settings.json` D-HOOKS read

ADR 0032 accepts a coherence cost: in a worktree with `ARCHKIT_ARCH_DIR` set, `D-INTENT-*` describes the named project's goals while `D-HOOKS` describes the local checkout's settings, in one report. The cure is disclosure, and it is now paid.

- **When the two roots agree**, the detail names the file and stops — `All guardrail hooks wired. Project settings: .claude/settings.json.` A path segment, not a sentence; the `[hooks]` warning text is byte-identical to before, so the common case is not made noisier.
- **When they diverge**, the detail switches to absolute paths and states the split outright: it names the checkout it read, names the `.arch/` every other check describes, says they are a different project, cites the ADR, and points the fix at the checkout's `settings.json` rather than the other tree. `ARCHKIT_ARCH_DIR` is named only when it is actually what moved the archDir.
- **Disclosed, never escalated.** `pass` is unchanged and no extra check row or warning appears. The same values ship structurally in the payload's `hooks` block (`projectSettingsPath`, `projectRoot`, `archProjectRoot`, `divergedFromArchDir`), and symlinked routes to the same project (`/var` → `/private/var`) are compared by real path — a false "two different projects" notice would teach the reader to ignore the true one.
...
### Added — lane integration: dispatch → converge → verify → land (ADRs 0023–0029)

CGR 2.0 (ADR 0013) could spawn parallel workers but had no sound way to bring them back. The loop is now closed at every step, and each step is derived from board events that survive `/clear`.

- **`dispatched` — a second live CGR state (ADR 0027).** It means *claimed on behalf of another session*. Because the MCP server resolves one `.arch/` from one cwd, a worker calling `archkit_goal_start` used to land `in-progress` in the **conductor's** shared state and re-arm the conductor's Stop-hook guard — telling it to work exit-criteria a worker was editing in a different worktree. `dispatched` **holds the lease** (`lease`, `dispatched-since`, `dispatched-to`) but is excluded from `GUARDED_STATUSES`, so the claiming session's guard releases while the lane stays live for conflict detection and drain checks. `archkit_goal_start` gains an optional `worker` — with it the claim *is* a dispatch; without it the tool behaves exactly as before. `startGoal` preserves `dispatched` rather than flipping it back; `startGoal(…, { reclaim: true })` is the explicit take-back after a failed dispatch.
- **The claim is now the rendered dispatch instruction (ADR 0028).** The conductor pass previously said only "claim + dispatch 1 worker/lane" with no call to make, so `dispatched` was implemented, tested, and never reached — conductors fell back to `archkit_goal_hold`, which frees the guard but drops the lease and misrepresents an actively-worked lane as deliberately parked (observed six times across two sessions). `dispatchClaims()` now derives the exact claims a pass owes (one per lane, and per *slug* for solo barriers so two barriers never collide onto one worker id), surfaced as `conductorPlan().dispatch`. Step 2 of the rendered pass emits `archkit_goal_start {slug, worker}` **before** the spawn, and marks `archkit_goal_hold` as the ✗ anti-pattern rather than offering it.
- **A completed dispatch now reaches the merge queue (ADR 0029).** Only fission ever appended a `completed` board event, so a CGR a worker finished normally never entered the merge queue and stayed pinned in `in_flight`. New `recordCompletion()` appends it — **gated on a prior claim**, so a foreground CGR with no worktree and no branch never manufactures a phantom integration point. The event carries what the archived CGR file takes with it (lane, worker, completion, `depends_on`, owned paths, `verify-command`), because completion moves the file to `done/`, which `loadGoal` deliberately doesn't read. Idempotent, and best-effort — a board failure never blocks marking a goal done.
- **Lane convergence before the drain (ADR 0023).** Worktree workers branch from a **stale base**, so draining the merge queue as N independent merges let merge #2 silently revert what merge #1 had just landed. `laneConvergencePlan()` groups the dependency-ordered queue **by lane** — one integration point per lane instead of one per CGR — and every group carries an explicit **rebase-onto-branch-tip precondition** so a merge can only fast-forward or conflict. The **path-extract fallback** (`git checkout <lane-branch> -- <owned paths>`) is bounded by the lane's declared ownership, for the case where the base is unrecoverably stale. Cross-lane dependency edges are induced from CGR-level `depends_on` and Kahn-sorted; a genuine cross-lane cycle degrades to ordered *segments* of the flat queue and flags `split: true` rather than mis-ordering silently. The word is **convergence**, never "reconcile" — that name is already taken by goal-*placement* reconcile (ADR 0020/0021), and a test asserts the emitted plan contains no `reconcil*` substring.
- **A concrete verify command per lane, with its outcome recorded (ADR 0024).** "Verify after each" named no command and recorded no result. `resolveVerifyCommand()` resolves one per CGR and unions it per lane — the CGR's own `verify-command`, else the detected project test command, else none — and each integration point carries `integration.verify`, `verifySource` (`cgr|project|mixed|none`) and `verifiable`. A lane whose CGRs resolve differently gets the de-duped union joined with `&&`. New **`archkit_board_merged`** appends a `merged` event per CGR whose status is **derived, never trusted**: no command → `unverified`, passed → `green`, failed → `red`, command with no reported result → `unverified`. Silence can no longer read as success. `sessionState` gained a `merged` slice and `conductorPlan` gained `unverifiedMerges`, so integration debt keeps the conductor non-idle.
- **PR-gated bucket landing (ADR 0025).** A drained bucket's `mergeGuidance` was unconditionally a direct `git merge` to mainline — bypassing the exact CI gate `RELEASING.md` promises. Landing is now gated on `cgr.finalize.ciCd`: **no provider** keeps the direct merge byte-for-byte (a local-only repo is a legitimate archkit user), **github-actions** emits `git push -u` + `gh pr create` with an explicit "wait for the required checks — the PR *is* the gate" clause, and any other provider gets the push plus a provider-agnostic open-the-PR instruction. `bucketCompletion` gained structured `landing` and `ciCd` fields.
- **Tier-3 conflict escalation.** New **`archkit_board_conflict`** mints a solo `merge-reconcile-*` barrier CGR for two lanes whose file content genuinely collides; `conductorPlan.pendingEscalations` surfaces collisions not yet escalated.

### Changed — MCP output is a graph, not prose (ADR 0026)

- **One shared renderer, `src/lib/format.mjs`.** The conductor prompt is re-emitted after every `/clear` and is the first thing in a fresh conductor's window — the most expensive context archkit owns, and it was spending ~900 characters of numbered prose on a board that five lines of graph render better. Board state now renders as a **tree** (lanes branch, goals leaf, barriers marked `⊘ … **SOLO**`) with counts as a stat strip. Repeated per-integration-point boilerplate is factored into **one substitution template**, so rendering cost is O(1) in lanes instead of O(lanes).
- **Four severities, defined once.** `▸ ! ✗ ✓` (action / attention / error / ok) — there is no fifth. Structural glyphs (`├─ └─ │ ⊘ → ⇠ ·`) live in a separate table because they carry shape, not urgency, and a one-line legend ships with any output that uses them. An unknown severity degrades to `action` rather than throwing — a rendering bug must never take a prompt down.
- **Markdown emphasis only — never ANSI.** MCP results are transported as plain text into an agent's context, where an escape sequence surfaces as literal garbage (`←[32m`). `tests/cgr-output-contract` asserts the ESC byte appears in no prompt result on any board shape, and not even in `format.mjs`'s own source. Terseness kept every instruction the prose carried; what it dropped is connective prose and restatement.
- **MCP tool descriptions halved.** Descriptions load into every agent's context whether or not the tool is called, so they are now held to a written contract (lead line → call-shaping detail only → `Trigger:` → disambiguation pointer) and a **800-byte-per-description ceiling**, enforced by `tests/mcp-tool-descriptions`. Rationale and history moved to the ADRs the descriptions now cite by number; detail with no ADR home moved to the new **`docs/mcp-tool-surface.md`**. 49 tools, 50 581 → 24 326 bytes total, largest 2 821 → 796 — a **52% cut** with no capability change.

### Added — finalize can cut a taggable commit

- **Version-bump step (`cgr.finalize.steps.version`).** `check:versions` enforces `package.json` == `.claude-plugin/plugin.json`, and `release.yml` refuses to publish unless the git tag equals `package.json`'s version — but no step in the CGR lifecycle ever bumped them, so every release needed out-of-band manual surgery. The new step is **default off** and ordered **first**, so the changelog entry and the commit message both describe the version being cut. Enabled, it expands to two exit-criteria — bump every file the project's own version check compares, then re-run that check before the commit step — and the barrier declares the bumped manifests in `owns`. `detectVersionSync` reads the project's check script and the manifests on disk rather than guessing, and degrades to a generic criterion when it finds nothing. Configs written before the step read `version: false` through the existing per-step default, so their criteria are byte-identical.

### Fixed

- **Intake round-tripped exit-criteria wrong.** YAML-ambiguous frontmatter values (a criterion starting `- yes`, `no`, `on`, a bare `:` …) were written unquoted and came back as booleans or parse errors; they are now quoted on write so criteria survive the round trip verbatim.
- **The finalize barrier landed on the wrong branch.** It didn't inherit the batch's `project`, so a `feat/<project>` batch's wrap-up goal was filed as ungrouped queue work and committed to the wrong branch.
- **Goal payloads named a path that didn't exist.** `archkit_goal_payload` emitted a synthesized path rather than the real on-disk location, so "read first" pointed at nothing after a placement reconcile. It now emits the actual path.
- **A finished dispatch aged toward reclaim.** `sessionState`'s `in_flight` fold now skips a claimed slug whose CGR has reached a terminal status — status is the source of truth (ADR 0003) — so a completed dispatch leaves `in_flight` and its lease stops aging into `leases_expired`.
- **Tests could write to the live `.arch/` board.** Every suite now runs in a sandboxed cwd; a repo-wide audit (`tests/spawn-cwd-audit`) fails the build on any test spawn without an explicit `cwd`, and 11 remaining cwd-less spawns plus a `__dirname`-relative bin resolution in `migrate-playbooks` were fixed. A new write guard attributes any `.arch/` board write **at its source** instead of blaming whichever suite happened to be running.

### Docs

- **`RELEASING.md` documents Trusted Publishing (OIDC)** instead of `NPM_TOKEN`, including the filename-binding trap (the npm publisher's workflow field must be the workflow **filename**, e.g. `release.yml`, not its display name).
- **New `docs/mcp-tool-surface.md`** — the tool-description contract, the byte ceiling, and the detail relocated out of descriptions.
