...
const IN_PROJECT_CONTEXT = [
  ...
  "",
  "Other archkit MCP tools available (25 total):",
  ...
  "",
  "CGR (Clear Goal Run) — the goal relay loop. When the user gives a sprawling or multi-part ask, call archkit_goal_intake to decompose it into discrete goals (one per fresh context). To advance the queue, tell the user to run /clear then the slash command /mcp__archkit__goal_next — that injects the next goal automatically (no copy-paste). You CANNOT run /clear or /mcp__archkit__goal_next yourself — they are the user's keystrokes; your job is to instruct them. While a goal is in-progress the Stop hook will block stopping until you call archkit_goal_complete <slug>, which releases the guard and advances the queue. Relay tools: archkit_goal_intake, archkit_goal_list, archkit_goal_show, archkit_goal_payload (fallback: paste after /goal), archkit_goal_verify (evidence before completing), archkit_goal_complete, archkit_goal_abandon (drop without completing).",
].join("\n");
